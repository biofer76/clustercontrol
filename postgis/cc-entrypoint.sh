...
# Connection to clustercontrol host timeout
TIMEOUT=60

wait_for() {
  local host=$1
  for i in `seq $TIMEOUT` ; do
    HTTP_CODE=`curl -I http://$host/clustercontrol/ 2>/dev/null | head -n 1 | awk '{print $2}'`
    if [ $HTTP_CODE ]; then
...
if [ -z "$CC_HOST" ]; then
  ...
  echo "Connection attempt, timeout: $TIMEOUT seconds"
